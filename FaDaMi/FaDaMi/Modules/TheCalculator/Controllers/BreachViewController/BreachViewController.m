...
- (void)cycleCalendarYearInterestRates:(NSMutableArray *)arrays {
    ...
    if (startTimeInt > [QZManager timeToTimeStamp:[self.timeArrays lastObject]]) {
        ...
        lastMoney = [self accordingOfDaysLookingForRatesWithRateArrays:rateArrays withDays:days withInterval:days withMoney:money];
        ...
        if (endTimeInt > lastObj) {
            ...
            for (NSInteger i = startIndex; i <= endIndex; i++) {
                ...
                float calculateMoney = [self accordingOfDaysLookingForRatesWithRateArrays:rateArrays withDays:days withInterval:differTimeDay withMoney:money];
...
- (float)accordingOfDaysLookingForRatesWithRateArrays:(NSArray *)rateArrays
                                             withDays:(float)allDays
                                         withInterval:(float)differTimeDay
                                            withMoney:(float)money {
    
    float selectRate = 0.0f;
    if(allDays <= 180){
        
        selectRate = [rateArrays[0] floatValue];
        _reatString = rateArrays[0];
    }else if(allDays <= 365){
        
        selectRate = [rateArrays[1] floatValue];
        _reatString = rateArrays[1];
    }else if(allDays <= 1095){
        
        selectRate = [rateArrays[2] floatValue];
        _reatString = rateArrays[2];
    }else if(allDays <= 1825){
        
        selectRate = [rateArrays[3] floatValue];
        _reatString = rateArrays[3];
    }else if(allDays > 1825){
        
        selectRate = [rateArrays[4] floatValue];
        _reatString = rateArrays[4];
    }
    return (selectRate / 360.f) * differTimeDay * money;
}
...
- (UIView *)tableView:(UITableView *)tableView viewForFooterInSection:(NSInteger)section
{
    ...
    UIView *secitionView = [[UIView alloc] initWithFrame:CGRectMake(0, 0, SCREENWIDTH, 80)];
...
- (UIButton *)calculateButton
{
    if (!_calculateButton) {
        ...
        _calculateButton.frame = CGRectMake(15 , 20, (SCREENWIDTH - 45)/2.f, 40);
...
- (UIButton *)resetButton
{
    if (!_resetButton) {
        ...
        _resetButton.frame = CGRectMake(30 + (SCREENWIDTH - 45)/2.f , 20, (SCREENWIDTH - 45)/2.f, 40);
...
- (UIView *)bottomView
{
    if (!_bottomView) {WEAKSELF;
        _bottomView = [[UIView alloc] initWithFrame:CGRectMake(0, 0, SCREENWIDTH, 75.f)];
        _bottomView.backgroundColor = [UIColor clearColor];
        UILabel *label2 = [[UILabel alloc] initWithFrame:CGRectMake(10, 35, SCREENWIDTH-10, 25)];
        ...
        UILabel *label1 = [[UILabel alloc] initWithFrame:CGRectMake(0, 0, SCREENWIDTH, 35)];
