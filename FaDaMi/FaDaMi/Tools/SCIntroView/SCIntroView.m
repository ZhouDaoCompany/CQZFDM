...
- (void)setUp {
    ...
    switch (self.introViewDoneMode) {
        ...
        case SCIntroViewDoneModePanGestureWithAnimation:
            self.scrollView.contentSize = CGSizeMake(SCREENWIDTH*(self.contentImages.count+1), self.scrollView.frame.size.height);
            break;
        default:
            self.scrollView.contentSize = CGSizeMake(SCREENWIDTH*self.contentImages.count, self.scrollView.frame.size.height);
            break;
    }
...
- (UIButton *)doneButton {
    if ([self.dataSource respondsToSelector:@selector(doneButtonInIntroView:)]) {
        ...
        if (CGPointEqualToPoint(doneButton.frame.origin, CGPointZero)) {
            float widthx = SCREENHEIGHT - self.frame.size.height*self.doneButtonLocation;
            doneButton.frame = CGRectMake(widthx+10.f, self.frame.size.height*self.doneButtonLocation, SCREENWIDTH - 2*widthx-20.f, 40);
        }
