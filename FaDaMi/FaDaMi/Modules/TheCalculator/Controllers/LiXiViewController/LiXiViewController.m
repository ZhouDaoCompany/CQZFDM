...
- (void)cycleCalendarYearInterestRates:(NSMutableArray *)arrays {
    ...
    [self collectInformationToAnotherInterfaceWithMoney:money WithBreachMoney:allMoney WithAllDays:allDays WithArrays:self.detailArrays WithReatType:_reatString];
}
...
- (void)waitingForTheForehead:(NSMutableArray *)arrays withIsBank:(BOOL)isBank
{
    ...
    double everyMonthMoney = [self loanPrincipal:money withAnInterest:rate withRepaymentPeriods:monthsCount];
    
    double allMoney = everyMonthMoney * monthsCount/1.f;
    ...
    if (isBank == YES) {
        ...
        for (NSMutableArray *smallArr in self.detailArrays) {
            ...
            [smallArr addObject:CancelPoint2(everyMonthMoney)];
        }
...
- (void)standOfTheBankInterestRatesOverTheSame:(NSMutableArray *)arrays {
    ...
    for (NSUInteger i = 0; i< monthsCount; i++) {
        ...
        if (i < [_detailArrays count]) {
          
            NSMutableArray *smallArrays = _detailArrays[i];
            float lixi = 0.0f;
            ...
            [tempArrays addObject:smallArrays];
        }
    }
    ...
    //收集信息
    NSString *allDays = [NSString stringWithFormat:@"%@-%@",[QZManager changeTimeMethods:[_startTime integerValue] withType:@"yy/MM/dd"],[QZManager changeTimeMethods:[_endTime integerValue] withType:@"yy/MM/dd"]];
    [self collectInformationToAnotherInterfaceWithMoney:allMoney WithBreachMoney:allMoney WithAllDays:allDays WithArrays:tempArrays WithReatType:_reatString];
}
