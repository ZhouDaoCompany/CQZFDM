...
- (UILabel *)addresslab{
    ...
    if (!_addresslab) {
        _addresslab = [[UILabel alloc] initWithFrame:CGRectMake(SCREENWIDTH -200, 12, 170, 20)];
...
- (UIImageView *)headImg{
    ...
    if (!_headImg) {
        _headImg = [[UIImageView alloc] initWithFrame:CGRectMake(SCREENWIDTH - 90, 10, 60, 60)];
...
- (UISwitch *)switchButton{
    ...
    if (!_switchButton) {
        _switchButton = [[UISwitch alloc] initWithFrame:CGRectMake(SCREENWIDTH-65.f, 7.f, 0, 30)];
...
- (void)settingUIWithSection:(NSUInteger)section withRow:(NSUInteger)row {
    
    
    _section = section; _row = row;
    if (_section == 0) {
        ...
        if (_row == 0) {
            ...
            _lineView.frame = CGRectMake(15, 79.5, SCREENWIDTH - 15, .5f);
            _headImg.hidden = NO;
            _addresslab.hidden = YES;
        }else{
            _nameLab.frame = CGRectMake(15, 12, 120, 20);
            _lineView.frame = CGRectMake(15, 43.5, SCREENWIDTH - 15, .5f);
            _headImg.hidden = YES;
...
- (void)thirdLoginActionWithPlatformType:(UMSocialPlatformType)platformType
    ...
    [[UMSocialManager defaultManager] getUserInfoWithPlatform:platformType currentViewController:_ParentView completion:^(id result, NSError *error) {
        if (error) {
...
- (void)removeTheBinding:(UISwitch *)switchBtn { WEAKSELF
