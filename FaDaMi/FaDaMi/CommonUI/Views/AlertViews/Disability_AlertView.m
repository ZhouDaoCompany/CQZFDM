...
{
    self = [super initWithFrame:SCREENFRAMERECT];
    if (self) {
...
- (UIView *)tableView:(UITableView *)tableView viewForHeaderInSection:(NSInteger)section
{
    if (_type == CaseType) {
        ...
        ConsultantHeadView *headView = [[ConsultantHeadView alloc] initWithFrame:CGRectMake(0, 0, SCREENWIDTH, 36.f) withSection:section];
...
- (UIView *)zd_superView
{
    if (!_zd_superView) {
        ...
        if (_type == SelectOnly) {
             ...
             height = (SCREENHEIGHT >568)?495.f:(SCREENHEIGHT - 60.f);
        }else{
             height = (SCREENHEIGHT >667)?(SCREENHEIGHT - 142.f):(SCREENHEIGHT - 60.f);
        }
        
        _zd_superView = [[UIView alloc] initWithFrame:CGRectMake((SCREENWIDTH - kContentLabelWidth)/2.f,SCREENHEIGHT, kContentLabelWidth, height)];
